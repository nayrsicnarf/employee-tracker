...
function addNewEmployee(db, cb) {
    const listRole = [];
    db.query("SELECT title FROM employeeRole", (err, results) => {
        if (err) {
            console.log(err);
        }
    ...
    inquirer
        ...
        .then((data) => {
            const sql = "SELECT id from employeeRole WHERE title = ?";
